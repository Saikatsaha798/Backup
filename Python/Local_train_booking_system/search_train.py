...
def searchT(src: str, dest: str):
    train_data = PrettyTable(["S.No.", "Arrival Time", "Departure Time", "Train Number", "Train Name"])
        ...
        if i > time:
            for j in sorted(time_table[dest]):
                ...
                if j > i and time_table[src][i]["train_number"] == time_table[dest][j]["train_number"]:
                    ...
                    train_data.add_row([trains, arrival, ":".join(time_table[src][i]['departure'].split(":")[0:2]),
